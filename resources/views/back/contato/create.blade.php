    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                ...
                @if ($response)
                    <form class="md:flex w-full flex-col" method="POST" action="{{ route('contato.store') }}">
                        @csrf
                        <div class="mb-4 ml-4 md:w-1/2">
                            <x-input-label for="cep" :value="__('CEP')" />
                            <x-text-input id="cep" class="block w-full" type="text" name="cep"
                                value="{{ old('cep') ? old('cep') : (isset($response['cep']) ? $response['cep'] : '') }}"
                                readonly />
                        </div>
                        <div class="md:flex mb-2">
                            <div class="md:w-1/2 ml-4">
                                ...
                                <x-text-input id="logradouro" class="block w-full" type="text" name="logradouro"
                                    value="{{ old('logradouro') ? old('logradouro') : (isset($response['logradouro']) ? $response['logradouro'] : '') }}"
                                    readonly />
                            </div>
                            <div class="ml-2 mr-4">
                        ...
                        <div class="md:w-1/2 ml-4">
                            ...
                            <x-text-input id="email" class="block w-full" type="email" name="email"
                                value="{{ old('email') }}"/>
                                <x-input-error :messages="$errors->get('email')" class="mt-2" texto="" />
