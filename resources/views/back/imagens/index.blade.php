    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <x-message></x-message>
                @if (request()->routeIs('imagem.index'))
                    <form class="md:flex w-full" method="POST" enctype="multipart/form-data" action="{{ route('imagem.store') }}">
                        @csrf
                        <div class="m-4">
                            <x-input-label for="tipo" :value="__('TIPO')" />
                            <x-select-input id="tipo" class="block w-full" name="tipo" :options="$tipos"
                                :value="old('tipo')" required />
                            <x-input-error :messages="$errors->get('tipo')" class="mt-2" texto="" />
                        </div>                        <div class="m-4">
                            <x-input-label for="imagem" :value="__('Imagem da Imagem')" />
                            <x-text-input id="imagem" class="block w-full" type="file" name="imagem"
                                 accept="image/jpeg, image/png"/>

                            <x-input-error :messages="$errors->get('imagem')" class="mt-2" texto=""/>
                        </div>
                ...
                @else
                    <form class="md:flex w-full" method="POST" enctype="multipart/form-data" action="{{ route('imagem.update', $imagem->id) }}">
                        @method('PUT')
                        @csrf
                        <div class="m-4">
                            <img src="{{asset('images/'.$imagem->imagem)}}" width="120">
                        </div>
                        <div class="m-4">
                            <x-input-label for="tipo" :value="__('TIPO')" />
                            <x-select-input id="tipo" class="block w-full" name="tipo" :options="$tipos"
                                :value="$imagem->tipo" required />
                            <x-input-error :messages="$errors->get('tipo')" class="mt-2" texto="" />
                        </div>
                        <div class="m-4">
                            <x-input-label for="imagem" :value="__('Imagem da Imagem')" />
                            ...
                            <div class="overflow-hidden">
                                            ...
                                            @foreach ($imagens as $item)
                                                <tr
                                                    ...
                                                    <td
                                                        ...
                                                        <img width="120" src="{{asset('images/'.$item->imagem)}}"></td>
                                                    <td
                                                    ...
                                                    <td
                                                        ...
                                                        <a href="{{ route('imagem.edit', $item->id) }}"
                                ...
                                @else
                                    <div class="w-full text-center">
                                        Nenhum dado encontrado em Imagens.
                                    </div>
                                @endif
                            </div>
                        </div>
